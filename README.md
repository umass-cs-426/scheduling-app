...
- Each branch corresponds to one of the following:
  ...
  - `lecture-NUM-NAME`: Contains lecture code, slides, and notes for lecture `NUM-NAME`.
  - `activity-NUM`: Contains an in-class activity for lecture `NUM`.
  - `exercise-NUM`: Contains an exercise for lecture `NUM`.
  - `homework-NUM`: Contains details for homework assignment `NUM`.
  - `projects`: Contains project information and related materials.

At least this is the idea for now :-)
...
The best way to work with any branch is to create your own local branch off of it. For example, if you are working on the `exercise-1` branch, you can create a new branch called `my-exercise-1`:

```bash
git checkout -b my-exercise-1 exercise-1
```

This creates a new branch called `my-exercise-1` based on the `exercise-1` branch. You can then make changes and commit them to your local branch without affecting the original `exercise-1` branch. This is also git best practice when working on any project with git.

When you are ready to save your changes, you can commit them using the following commands:
...
## Opening a Slide Deck

We use slidev to create and present slide decks for lectures. To open a slide deck, first ensure you are on the correct branch for the lecture you want to view (e.g., `lecture-1.2-the-monolithic-system-exists`). Then, run the following command in the terminal:

```bash
npm slides
```

This will start a local server and open the slide deck in your default web browser. You can navigate through the slides using the arrow keys or by clicking on the slides.
